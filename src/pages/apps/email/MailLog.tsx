...
// ** Types
import { MailType, MailLogType, MailLabelType, MailFolderType, MailFoldersArrType } from './types'
      ...
      >
        ...
        <Box sx={{ py: 2, px: { xs: 2.5, sm: 5 } }}>
          <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
            <Box sx={{ display: 'flex', alignItems: 'center' }}>
              {store.mails && store.selectedMails ? (
                <Checkbox
        ...
        <Box sx={{ p: 0, position: 'relative', overflowX: 'hidden', height: 'calc(100% - 7.25rem)' }}>
          <PerfectScrollbar options={{ wheelPropagation: false, suppressScrollX: true }}>
            {store.mails !== null && store.mails.length ? (
              <List sx={{ p: 0 }}>
