...
import { KanbanTasksProps } from 'src/pages/apps/kanban/types'
...
const KanbanTasks = (props: KanbanTasksProps) => {
  ...
  const renderAttachmentsComments = () => {
    if (task.attachments.length || task.comments.length) {
      return (
  ...
  const renderTaskFooter = () => {
    return task.attachments.length || task.comments.length || task.assignedTo.length ? (
      <Box
        sx={{
          mt: 4,
          display: 'flex',
