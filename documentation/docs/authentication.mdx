...
<p>
  ...
  JWT without removing ours or modifying our JWT config.
</p>
...
<ul className='mb-5'>
  <li>
    <strong>uswJwt.js:</strong> this file holds composition function <code>useJwt</code>. <code>useJwt</code> function
  ...
  <li>
    ...
    passing your config object in <code>useJwt</code> composition function.
  </li>
  <li>
...
<p className='mb-5'>
  We use axios response interceptor to auto-renew the JWT token once it gets expired. We tried our best to provide this
...
<p>
  ...
  update. So, you can use the composition function we provided to use JWT. You can find your JWT files in{' '}
  <code>src/auth/jwt</code>. In <code>useJwt.js</code> file instead of passing blank objects you can pass your own
  config to configure the JWT service. You can refer to the default config to check which options are available to
  override.
</p>

<p>We haven't provided a config for storing user data because it is up to you how to save user data.</p>

<p className='mb-5'>
  You will also find auth utils in <code>src/auth/utils.js</code> file. Those utils are well commented and
...
<p>
  Certainly, you might not want to use localStorage to save tokens as it is not much secure and you want to use cookies
